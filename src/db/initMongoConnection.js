...
export const initMongoDBConnection = async () => {
  try {
    ...
    console.log('MONGODB_DOMAIN:', domain);
    // const connectionURI = `mongodb+srv://${user}:${password}@${domain}/${db}?retryWrites=true&w=majority&appName=chat-app-cluster`;

    const connectionURI = `mongodb+srv://${user}:${encodeURIComponent(
      password,
    )}@${domain}/${db}?retryWrites=true&w=majority&appName=chat-app-cluster`;
